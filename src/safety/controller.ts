...
export class SafetyController extends EventEmitter implements SafetyControllerInterface {
  ...
  private recentEvents: SafetyEvent[] = [];
  private readonly MAX_ABANDONED_RATE = 0.03; // 3% max abandoned rate
  private readonly MAX_FAILED_RATE = 0.2; // 20% max failure rate
  private readonly PROGRESSIVE_FALLBACK_THRESHOLD = 0.5;
  ...
  constructor(private readonly config?: {
    ...
    if (config?.maxAbandonedRate !== undefined) {
      (this as any).MAX_ABANDONED_RATE = config.maxAbandonedRate;
    }
    if (config?.maxFailedRate !== undefined) {
      (this as any).MAX_FAILED_RATE = config.maxFailedRate;
    }
    if (config?.progressiveFallbackThreshold !== undefined) {
      (this as any).PROGRESSIVE_FALLBACK_THRESHOLD = config.progressiveFallbackThreshold;
    }
  }
  ...
  private calculateSafetyMargin(context: SafetyContext): number {
    let margin = 1.0;
  ...
  private recordDecision(decision: SafetyDecision): SafetyDecision {
